package com.itcodebox.fxtools.utils.SVGs;
...
public class SVGUtil {
	...
	private static void parseInfo(SvgXml svgXml, Document document) {
		...
		if(attrWidth==null){
			svgXml.setWidth(PluginConstant.NotFount);
		}else{
			svgXml.setWidth(attrWidth.getNodeValue());
		}

		Node attrHeight = attrs.getNamedItem("height");
		if(attrHeight==null){
			svgXml.setHeight(PluginConstant.NotFount);
		}else{
			svgXml.setHeight(attrHeight.getNodeValue());
		}
	}
